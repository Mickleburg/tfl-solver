...
Запуск: `py -3 tools/eval_suite.py`. Тест `tests/test_eval_suite.py`
...
def _lab1_variants():
    ...
    return status, (
        ...
        f"{counts[True] + 5} (`py -3 tools/lab1_termination.py`)"
    )
...
def _lab1_invariants():
    ...
    return SOLVED, (
        ...
        "`py -3 tools/lab1_invariants.py`, там же граница: у варианта 20 "
...
# --------------------------------------------------------------------------
# Экзамен, вопрос 2 — LL/LR и детерминированность языков
# --------------------------------------------------------------------------


def _exam2_ticket_1_ll_language():
    """Билет 2025 № 1: ``S -> S ab S | S c | ε`` задаёт LL-язык.

    Исходная грамматика леворекурсивна и сама не LL(1), но её язык равен
    ``(ab|c)*``.  В eval важно различать свойство предъявленной грамматики и
    существование LL-грамматики для того же языка.
    """
    from tfl.cfg import parse_cfg
    from tfl.parse import language

    source = parse_cfg("S -> S a b S | S c | ε")
    witness = parse_cfg("T -> a b T | c T | ε")
    if source.is_ll1():
        return "ошибка", "исходная леворекурсивная грамматика неожиданно признана LL(1)"
    if not witness.is_ll1():
        return "ошибка", "регулярная грамматика-свидетель неожиданно не LL(1)"
    if language(source, 10) != language(witness, 10):
        return "ошибка", "языки исходной грамматики и LL(1)-свидетеля разошлись до длины 10"
    return SOLVED, (
        "да: язык равен (ab|c)* и задаётся LL(1)-грамматикой "
        "T → abT | cT | ε; исходная грамматика при этом не LL(1), поскольку "
        "она леворекурсивна"
    )
...
def _exam_error_dfa():
    """Часть 2 экзамена 2026: найти ошибку в готовом ИИ-решении.

    В проверяемом решении утверждается ``a*b* = (a|b)*``, потому что оба
    языка состоят из букв ``a`` и ``b``. Точный контрпример обязан показывать
    не просто различие записей, а разные ответы распознавателей.
    """
    from tfl.automata import counterexample, dfa_of

    expected = dfa_of("a*b*", "ab")
    claimed = dfa_of("(a|b)*", "ab")
    witness = counterexample(expected, claimed)
    if witness is None:
        return "ошибка", "ошибочная и правильная регулярки неожиданно эквивалентны"
    if expected.accepts(witness) or not claimed.accepts(witness):
        return "ошибка", f"свидетель {witness!r} не различает языки в нужную сторону"
    return SOLVED, (
        f"первая ошибка — подмена a*b* на (a|b)*; точный минимальный "
        f"контрпример {witness!r} принимается заявленной регуляркой и "
        "отвергается исходной"
    )
...
CASES: tuple[Case, ...] = (
    ...
    Case("exam2-билет-1-ll-язык", "EXAM-2", "экзамен 2025, билет 1, вопрос 2",
         "задаёт ли S → SabS | Sc | ε LL-язык", SOLVED, _exam2_ticket_1_ll_language),
    ...
    Case("exam-ошибка-в-ии-решении", "EXAM-ERROR", "формат экзамена 2026, часть 2",
         "найти первую ошибку в решении a*b* = (a|b)*", SOLVED, _exam_error_dfa),
