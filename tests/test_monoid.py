...
Алгоритм реализован с существенной проверкой: считать моноид только по
полезным состояниям нельзя, поскольку функции становятся частичными.
Синтаксическим такой моноид не является — см.
`test_trap_state_changes_the_monoid`.
"""
