...
# --------------------------------------------------------------------------
#
# Зафиксированные контрольные варианты: совпадение не заменяет доказательство,
# но расхождение означает регрессию или ошибку формализации.


def test_reference_variant_15_has_17_classes():
    """Вариант 15: таблица классов содержит 17 строк.

    Расхождение 16 против 17 при первом прогоне оказалось состоянием-ловушкой:
    в таблице это строка `aabaa` из одних минусов. С учётом ловушки
...
def test_reference_variant_2_has_22_states():
    """Вариант 2: минимальный ДКА содержит 22 состояния."""
    dfa = dfa_of(
