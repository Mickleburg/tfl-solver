...
Формализм из дополнительной лекции
(`corpus/txt/Supplement_Alternating_Automata.txt`): автомат
...
В ЛР2 ПКА удобно понимать через lookahead и инварианты регулярного языка.

Отсюда две конструкции, которыми ПКА и строится на практике:

* `conjunction` — И-ветвление в стартовой вершине: язык распадается
  в конъюнкцию простых условий (первый контрольный пример,
  `^(?= .* a .* $) .* b .* $`);
* `with_lookahead` — **рекурсивный** инвариант: в некоторых состояниях
  ...
  (второй контрольный пример, `^((?=(b*ab*ab*)*$)a*b)*$`).
...
class AFA:
    ...
    def summary(self) -> str:
        ...
        ПКА может оказаться не меньше ДКА: ценность построения тогда
        определяется явно сформулированным нетривиальным инвариантом.
        """
        nfa = self.to_nfa()
...
def conjunction(parts: Sequence[DFA], alphabet: Iterable[str] | None = None) -> AFA:
    ...
    Первый контрольный пример: `^(?= .* a .* $) .* b .* $` — конъюнкция
...
def with_lookahead(
    ...
    второй контрольный пример `^((?=(b*ab*ab*)*$)a*b)*$`, где по `b`
