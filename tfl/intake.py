...
встречается и в РК1, и в билете, и в «Аптеке». Поэтому форма берётся из
пометки пользователя, а не угадывается; без пометки она просто неизвестна,
и это не мешает выбрать метод.
"""
...
FEATURES: dict[str, re.Pattern[str]] = {
    ...
    "атрибутная грамматика": re.compile(r"(?<!:):=|\w\.[ab]\b\s*[><=]"),
    ...
    "реверс": re.compile(r"[a-zA-Zω]\s*R\b|wR|\bреверс|обращени"),
...
def classify(text: str, hint: str | None = None, limit: int = 3) -> list[Candidate]:
    ...
    `hint` — пометка пользователя о форме контроля (`«РК2»`, `«ЛР3»`,
    `«экзамен»`, `«Аптека»`). Она сужает набор классов, но не подменяет
    разбор: если признаки указывают на другое, это будет видно по уликам.
