...
class SRS:
    ...
    def closure(
        ...
        Неисчерпывающее переписывание — типичная ошибка класса `RK2-A`,
        и ровно её этот метод снимает.
    ...
    def terminates(
        ...
        # Последняя попытка — рекурсивный путевой порядок. Слово читается
        # цепочкой одноместных термов (`tfl.trs.from_srs`), и LPO сравнивает
        # уже не длины, а вложенность: `ab → bba` он ориентирует, хотя правая
        # часть длиннее. На вариантах ЛР1 2025 не дал **ничего** сверх
        # интерпретаций — это замерено, а не предположено.
        from tfl.trs import from_srs

        lpo_order = from_srs(self).find_precedence()
        if lpo_order is not None:
            shown = " ≻ ".join(lpo_order.split())
            return Verdict(
                True,
                f"все правила убывают в рекурсивном путевом порядке "
                f"при старшинстве букв {shown}",
                witness=lpo_order,
            )
    ...
    def matrix_invariants(
        ...
        гомоморфизмом обходится контрольный вариант 20 ЛР1 2025, у которого
        нет ни счётных
