...
def build_report(pattern: str, title: str, outdir: pathlib.Path) -> str:
    ...
    add("")
    add("[ВРУЧНУЮ] Пункт требует содержательной догадки: надо заметить "
        "свойство языка, которое распадается в конъюнкцию нескольких простых "
        "условий, и построить автомат на их пересечении. Генератор такого "
        "не выдумывает.")
    add("")
    add("Порядок работы — см. `docs/recipes/LAB-2.md`, §3. Построенный "
        "автомат обязательно прогнать через "
        "`tfl.automata.disagreements` против ДКА.")
    add("")
    ...
    add("| ПКА против ДКА | — | [НЕ ПРОВЕРЕНО] пункт 3 не выполнен |")
